...
public class GeneratePropertyActionHandler
		...
		try {
			...
			if (displayMememberChooser(filteredFields.length, filteredMethods.length)) {
				...
				for (PsiField psiField : filteredFields) {
					if (psi.findAllFieldByName(clazz, StringUtil.generatePropertyConstantName(psiField.getName())) ==
							null) {
	 ...
	 */
	private ConflictResolutionPolicy exitsMethodDialog(PsiClass clazz,
