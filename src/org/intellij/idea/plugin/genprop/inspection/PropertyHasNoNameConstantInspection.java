 ...
 * @since 2.13
 */
public class PropertyHasNoNameConstantInspection extends AbstractToStringInsepction {
    ...
    public ProblemDescriptor[] checkClass(PsiClass clazz, InspectionManager im, boolean onTheFly) {
        if (log.isDebugEnabled()) log.debug("PropertyHasNoNameConstantInspection.checkClass: clazz=" + clazz + ", onTheFly=" + onTheFly);
		...
		for (int i = 0; i < dumpedFields.size(); i++) {
			...
			PsiField constantField = psi.findFieldByName(clazz, StringUtil.generatePropertyConstantName(field.getName()));
			...
			if(constantField == null) {
				...
				ProblemDescriptor problem = im.createProblemDescriptor(clazz, "Field has no property name constant", fix, ProblemHighlightType.GENERIC_ERROR_OR_WARNING);
