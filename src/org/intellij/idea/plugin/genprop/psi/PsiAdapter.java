...
public class PsiAdapter {
	...
	/**
	 * Get's the current class for the javafile and relative to the editor's caret. It's possible to get an inner class if
	 * the caret is positioned inside the innerclass.
	 *
	 ...
	 *
	 * @param element1 element to find it's class
	 *
	 * @return the class, null if not found.
	 */
	@Nullable
	public PsiClass findClass(PsiElement element1) {
	 ...
	 */
	public boolean isBooleanType(PsiElementFactory factory, @Nullable PsiType type) {
		assert type != null;
		if (isPrimitiveType(type)) {
	...
	/**
	 * Is the given field a numeric type (assignable from java.lang.Numeric or a primitive type of byte, short, int, long,
	 * float, double type)?
	 *
	 * @param factory element factory.
	 * @param type type.
	 ...
	 * @param modifiers the modifiers
	 */
	public boolean isModifierAbstract(@Nullable PsiModifierList modifiers) {
	 ...
	 *
	 * @throws com.intellij.util.IncorrectOperationException is thrown if there is an error creating the importstatement.
	 */
	 ...
	 */
	public boolean isTypeOfVoid(@Nullable PsiType type) {
	...
	public void executeCommand(Project project, Runnable runable) {
		CommandProcessor.getInstance().executeCommand(project, runable, "GenerateProperty", null);
	}
	 ...
	 *
	 * @param elem1 the element
	 *
	 * @return the parent method if found, <tt>null</tt> if not found.
	 */
	@Nullable
	public static PsiMethod findParentMethod(PsiElement elem1) {
		PsiElement elem = elem1;
		while (true) {
			if (elem == null) {
				return null;
			}
