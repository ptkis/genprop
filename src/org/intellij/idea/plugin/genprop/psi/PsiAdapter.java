...
public abstract class PsiAdapter {
    ...
    /**
     * Get's the PSIElementFactory. <p/>The factory can be used to create methods.
     *
    ...
    /**
     * Get's the current class for the javafile and relative to the editor's caret. It's possible to get an inner class
     * if the caret is positioned inside the innerclass.
     *
    ...
    /**
     * Finds the class for the given element. <p/> Will look in the element's parent hieracy.
     *
    ...
    /**
     * Returns true if a field is constant. <p/> This is identifed as the name of the field is only in uppercase and it
     * has a <code>static</code> modifier.
     *
    ...
    /**
     * Is the given field a numeric type (assignable from java.lang.Numeric or a primitive type of byte, short, int,
    ...
    public boolean isNumericType(PsiElementFactory factory, PsiType type) {
        if (isPrimitiveType(type)) {
            ...
            String s = type.getCanonicalText();
            return "byte".equals(s) || "double".equals(s) || "float".equals(s) || "int".equals(s) || "long".equals(s) ||
                    "short".equals(s);
        } else {
     ...
     *          is thrown if there is an error creating the
     *          importstatement.
     */
    public void addImportStatement(PsiJavaFile javaFile,
        ...
        JavaCodeStyleManager javaCodeStyleManager = JavaCodeStyleManager.getInstance(codeStyleManager.getProject());
        javaCodeStyleManager.optimizeImports(javaFile);
    }

    /**
     * Fixes the import statement to be returned as packagename only (without .* or any Classname). <p/> <br/>Example:
     * java.util will be returned as java.util <br/>Example: java.util.* will be returned as java.util <br/>Example:
     * java.text.SimpleDateFormat will be returned as java.text
     *
    ...
    /**
     * Does this class have a super class? <p/> If the class just extends java.lang.Object then false is returned.
     ...
     */
    public void optimizeImports(Project project, PsiJavaFile javaFile)
            throws IncorrectOperationException {
    ...
    /**
     * Removes the action from the menu. <p/> The group must be of a DefaultActionGroup instance, if not this method
     * returns false.
     *
    ...
    /**
     * Adds the action to the menu. <p/> The group must be of a DefaultActionGroup instance, if not this method returns
     * false.
     *
     ...
     * @param replace          true if any existing javadoc should be replaced. false will not replace any existing javadoc and
     *                         thus leave the javadoc untouched.
     * @return the added/replace javadoc comment, null if the was an existing javadoc and it should <b>not</b> be
     *         replaced.
     ...
     */
    @Nullable
    public PsiField findAllFieldByName(PsiClass clazz, String name) {
    ...
    /**
     * Is the method a getter method? <p/> The name of the method must start with <code>get</code> or <code>is</code>.
    ...
    /**
     * Get's the field name of the getter method. <p/> The method must be a getter method for a field. Returns null if
     * this method is not a getter. <p/> The fieldname is the part of the name that is after the <code>get</code> or
    ...
    public boolean isEnumField(PsiManager manager, PsiField field) {
        ...
        PsiClass clazz = psiFacade.findClass(name, scope);
        if (clazz == null) {
            return false;
        }
    ...
    public PsiMethod findEqualsMethod(PsiClass clazz) {
        ...
        for (PsiMethod method : methods) {
            ...
            if (!("java.lang.Object".equals(parameters[0].getType().getCanonicalText()))) {
                continue;
            }
     ...
     * @throws IncorrectOperationException is thrown if error adding/replacing the javadoc comment.
     */
    public PsiAnnotation addAnnotationToMethod(PsiElementFactory factory, PsiMethod method, String annotation)
            throws IncorrectOperationException {
     ...
     */
    public void addImplements(Project project, PsiClass clazz, String interfaceName)
            throws IncorrectOperationException {
        PsiManager manager = getPsiManager(project);
        ...
        JavaPsiFacade psiFacade = JavaPsiFacade.getInstance(project);
        PsiClass interfaceClass = psiFacade.findClass(interfaceName, GlobalSearchScope.allScope(project));
